...
class ThirdQuesFragment :Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.tasktwothird, container, false)
        TaskTwoSubActivity.stepView.go(2,true)
        btn = view.findViewById(R.id.nxt)
        editText = view.findViewById(R.id.editText)

        btn.setOnClickListener {
            if (allow)
                fragmentManager?.beginTransaction()?.replace(R.id.host_fragment2, FourthQuesFragment())?.commit()
        }
